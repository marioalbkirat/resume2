...
export const defaultGlobalStyle: StyleObject = { fontFamily: "Arial", fontSize: "14px", lineHeight: 1.5, color: "#111827", backgroundColor: "#ffffff", padding: "40px", margin: "0 auto" };
...
export function useVisualStylesPanel() {
  ...
  const updateStyle = (recipe: (previous: ResumeStyle) => ResumeStyle) => setStyle((previous) => recipe({ global: { ...defaultGlobalStyle, ...(previous.global ?? {}) }, selectors: previous.selectors ?? {}, elements: previous.elements ?? {}, customCSS: previous.customCSS ?? "" }));
  ...
  return { style: { ...style, global: { ...defaultGlobalStyle, ...(style.global ?? {}) } }, selectedNode, selectedGroup, themes, fonts, updateGlobal, updateSelector, updateElement, setColor, valueFor, applyTheme, resetStyles, applyStyles };
}
