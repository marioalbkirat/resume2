...
export default function SectionBuilderUI({ sectionName, schema, content, onExport }: SectionBuilderUIProps) {
    ...
    const builder = useSectionBuilder({ initialSchema: schema, initialContent: content, sectionName });
    const contentCount = Object.keys(builder.getAllContent()).length;

    useEffect(() => {
        onExport?.({ schema: builder.schema, content: builder.content });
    ...
    return (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
            ...
            {advancedMode ? <div className="p-4"><div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full"><div className="lg:border-r border-gray-200 lg:pr-6"><div className="flex items-center justify-between mb-3"><span className="text-sm font-medium text-gray-600">📁 Structure Tree</span><span className="text-xs text-gray-400">{contentCount} elements</span></div><div className="overflow-auto max-h-[60vh]"><TreeNode builder={builder} isRoot={true} /></div></div><div className="lg:pl-6"><div className="flex items-center justify-between mb-3"><span className="text-sm font-medium text-gray-600">👁️ Preview</span><span className="text-xs text-green-500 flex items-center gap-1"><span className="w-1.5 h-1.5 bg-green-500 rounded-full"></span>Live</span></div><div className="overflow-auto max-h-[60vh] p-4 bg-gray-50 rounded-lg border border-gray-200"><SectionPreview builder={builder} /></div></div></div></div> : <div className="p-8 text-center text-gray-600"><p className="text-lg font-medium text-gray-800">Describe the section you want.</p><p className="mt-2">Example: “Languages section with language name and proficiency level.”</p><button onClick={() => setShowAIModal(true)} className="mt-5 inline-flex items-center gap-2 px-5 py-3 bg-purple-600 text-white rounded-lg"><FiCpu />Open AI generator</button></div>}
