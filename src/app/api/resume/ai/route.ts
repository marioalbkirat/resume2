...
const prompts: Record<AITool, string> = {
    ...
    translation: `${jsonInstruction}\nTranslate the provided resume content into the requested target language while preserving professional resume tone, names, dates, structure, and meaning. Return: {"summary": string, "content": object, "changes": string[]}. The content object must preserve every original content key. For each entry, keep id, type, prop, and all non-text metadata unchanged; translate only human-readable value fields. Never return null for content.`,
    ...
    optimize: `${jsonInstruction}\nRewrite and optimize the full resume content. Return: {"summary": string, "content": object, "changes": string[]}. The content object must preserve every original content key. For each entry, keep id, type, prop, and all non-text metadata unchanged; update only human-readable value fields into complete, polished, truthful, ATS-friendly resume text. Never return null for content.`,
