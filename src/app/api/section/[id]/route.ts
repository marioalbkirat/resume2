...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        const result = await prisma.section.findUnique({ where: { id }, include: { user: { select: { email: true, isAdmin: true } } } });
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        const duplicate = await prisma.section.findFirst({ where: { authorId, name: raw.name, NOT: { id } }, select: { id: true } });
        if (duplicate) return NextResponse.json({ error: "You already have a section with this name." }, { status: 409 });
        const updated = await prisma.section.updateMany({ where: { id, authorId }, data: { ...raw, schema: raw.schema as never, content: raw.content as never } });
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        const { id } = await params;
        const deleted = await prisma.section.deleteMany({ where: { id, authorId: ADMIN_USER_ID, visibility: "OFFICIAL" } });
        if (!deleted.count) return NextResponse.json({ error: "Only official admin sections can be deleted." }, { status: 403 });
