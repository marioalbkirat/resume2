...
import { FiPlus, FiEye, FiEyeOff, FiCopy, FiFolder, FiMinus, FiX, FiTrash2 } from "react-icons/fi";
...
export default function SectionsPanel() {
    ...
    const { deleteSection } = useSectionServices();
    ...
    const handleDeletePrivateSection = async (section: Section) => {
        if (section.authorId !== CURRENT_USER_ID) return;
        const result = await Swal.fire({
            icon: "warning",
            title: "Delete private section?",
        ...
        try {
            ...
            removeSectionFromDistribution(section.id);
    ...
    const renderGalleryPanel = (title: string, items: Section[], allowDelete = false) => (
        <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                {items.map(section => {
                    ...
                    return <div key={section.id} className={`flex items-center gap-2 rounded-lg border p-2 transition-all ${added ? "bg-blue-50 border-blue-200" : "hover:border-blue-300 hover:bg-blue-50 border-gray-200"}`}>
                        <button type="button" onClick={() => added ? removeSectionFromDistribution(section.id) : addSectionToDistribution(section.id)} className="flex flex-1 cursor-pointer items-center gap-2 text-left">
                            <span className="text-sm flex-1">{section.name}</span>{added ? <FiMinus className="w-4 h-4 text-red-600" /> : <FiPlus className="w-4 h-4 text-blue-600" />}
                        </button>
                        {allowDelete && <button type="button" onClick={() => handleDeletePrivateSection(section)} className="rounded-md p-1.5 text-red-600 hover:bg-red-50" title="Delete private section" aria-label={`Delete ${section.name}`}><FiTrash2 className="h-4 w-4" /></button>}
                    </div>;
                })}
            </div>
        </div>
