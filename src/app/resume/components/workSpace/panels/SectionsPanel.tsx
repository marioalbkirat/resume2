...
import { FiPlus, FiEye, FiEyeOff, FiCopy, FiFolder, FiMinus } from "react-icons/fi";
...
export default function SectionsPanel() {
    ...
    const openDuplicate = (section: Section) => {
        const name = prompt("New section name", `${section.name} Copy`);
        if (!name?.trim()) return;
        if (!assertUnique(name)) return alert("You already have a section with this name.");
        setDuplicateSource(section); setDuplicateName(name); setShowCreateSection(true);
    };
    ...
    const selectedSections = sections.filter(s => distribution[s.id]).sort((a,b) => distribution[a.id].order - distribution[b.id].order);
