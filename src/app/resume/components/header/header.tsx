...
import { FiDownload, FiUpload, FiSave, FiLink, FiGrid, FiFileText, FiEdit3, FiEye, FiLayers, FiStar } from "react-icons/fi";
...
export default function ResumeHeader() {
    ...
    const saveDraft = async () => {
        if (!selectedResume || !draftTitle.trim()) return;
        const response = await fetch("/api/resume/resume-draft", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ title: draftTitle.trim(), templateId: selectedResume.id, content, schema: resumeDraftSchema, settings, distribution, style }),
        });
    ...
    return (
        <header id="resume-header" className="flex items-center justify-between px-6 py-3 bg-linear-to-r from-slate-50 to-blue-50/30 border-b border-slate-200 shadow-sm">
            ...
            <div className="flex items-center gap-2">
                ...
                <button onClick={() => setShowDraftDialog(true)} className="cursor-pointer flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-linear-to-r from-blue-600 to-indigo-600 rounded-lg hover:shadow-lg"><FiSave className="w-4 h-4" />Save draft</button>
