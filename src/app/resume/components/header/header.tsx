    ...
    return (
        <header id="resume-header" className="flex items-center justify-between px-6 py-3 bg-linear-to-r from-slate-50 to-blue-50/30 border-b border-slate-200 shadow-sm">
            ...
            <div className="flex items-center gap-2">
                ...
                <button type="button" onClick={downloadActiveResume} disabled={!activeDraft?.slug} className="cursor-pointer flex items-center gap-2 px-3.5 py-2 text-sm font-medium text-slate-700 bg-white rounded-lg border border-slate-200 disabled:cursor-not-allowed disabled:opacity-50"><FiDownload className="w-4 h-4" />Export</button><input ref={importInputRef} type="file" accept="application/pdf" onChange={importResumePdf} className="hidden" /><button type="button" onClick={() => importInputRef.current?.click()} disabled={isImporting} title={importStatus || "Import PDF resume/CV"} className="cursor-pointer flex items-center gap-2 px-3.5 py-2 text-sm font-medium text-slate-700 bg-white rounded-lg border border-slate-200 disabled:cursor-not-allowed disabled:opacity-50">{isImporting ? <FiLoader className="w-4 h-4 animate-spin" /> : <FiUpload className="w-4 h-4" />}Import</button><Link href="/dashboard" className="cursor-pointer flex items-center gap-2 px-3.5 py-2 text-sm font-medium text-slate-700 bg-white rounded-lg border border-slate-200"><FiGrid className="w-4 h-4" />Dashboard</Link>
            </div>
            {importStatus && <div className="absolute right-6 top-17 z-40 max-w-sm rounded-xl border border-blue-100 bg-white px-4 py-2 text-xs font-medium text-slate-600 shadow-lg">{importStatus}</div>}
