...
export default function Page() {
    ...
    useEffect(() => { (async () => { const id = params.id; if (typeof id !== "string") return; const data = await getSectionById(id); setSection(data); setBuilderData({ schema: data.schema, content: data.content }); setName(data.name); })(); }, [params.id, getSectionById]);
    const handleSubmit = async () => { if (!section || !builderData || typeof params.id !== "string") return; if (!name.trim()) return alert("Section name is required"); await updateSection(params.id, { name: name.trim(), target: "RESUME", visibility: "OFFICIAL", schema: { ...builderData.schema, name: name.trim() }, content: builderData.content }); router.push("/admin/section/"); };
